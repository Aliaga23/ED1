...
void PoliP::poner_termino(int coef, int exp)
{
    if (coef != 0 && exp >= 0) {
		...
		if (existe == NULL) { // Nueva Inserción
            NodoP* aux = new NodoP();
            if (aux != NULL) {
				aux->coef = coef;
                aux->exp = exp;

                // Agregar lógica para mantener el polinomio ordenado de mayor a menor
 ...
 /*  void PoliP::poner_termino(int coef, int exp)
{
	...
	if (ptr_poli == nullptr || exp > ptr_poli->exp) {
		// Caso especial: el nuevo término se convierte en el nuevo primer término.
		nuevo->sig = ptr_poli;
		ptr_poli = nuevo;
	} else {
		NodoP* actual = ptr_poli;
		...
		// Buscar el lugar adecuado para el nuevo término.
		while (actual != nullptr && exp < actual->exp) {
			anterior = actual;
			actual = actual->sig;
		}

		// Verificar si ya existe un término con el mismo exponente.
		if (actual != nullptr && exp == actual->exp) {
			// Sumar al coeficiente existente y eliminar el término nuevo.
			actual->coef += coef;
			delete nuevo;
		} else {
			// Insertar el nuevo término en su lugar adecuado.
			nuevo->sig = actual;
			anterior->sig = nuevo;
		}
	}
