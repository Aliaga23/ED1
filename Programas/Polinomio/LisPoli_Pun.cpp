...
float PoliLP::evalua(float x)
{
    float resultado = 0.0;
    direccionP actual = pol->primero(); // Comienza desde el primer término

    while (actual != NULL) {
        int coeficiente = pol->recupera(actual); // Obtiene el coeficiente
        int exponente =pol->recupera(pol->siguiente(actual)); // Obtiene el exponente

		resultado +=coeficiente * pow(x, exponente); // Suma el término al resultado

		actual = pol->siguiente(pol->siguiente(actual)); // Avanza al siguiente término
    }
